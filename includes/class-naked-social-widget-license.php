...
/**
 * License handler for Naked Social Widget
 *
...
/**
 * Class Naked_Social_Widget_License
 * 
 * @since 1.0
 */
class Naked_Social_Widget_License {
	...
	/**
	 * Naked_Social_Widget_License constructor.
	 *
	...
	public function __construct( $_file, $_item, $_version, $_author, $_optname = null, $_api_url = null ) {
		...
		$this->item_shortname = 'naked_social_widget_pages_' . preg_replace( '/[^a-zA-Z0-9_\s]/', '', str_replace( ' ', '_', strtolower( $this->item_name ) ) );
		$this->version        = $_version;
		$this->license        = trim( naked_social_widget_get_option( $this->item_shortname . '_license_key', '' ) );
		$this->author         = $_author;
	...
	private function hooks() {
		...
		// Register settings
		add_filter( 'naked-social-widget/settings/general', array( $this, 'settings' ), 1 );
		...
		add_action( 'naked-social-widget/events/weekly', array( $this, 'weekly_license_check' ) );
		...
		$ama_license_settings = array(
			'main' => array(
				$this->item_shortname . '_license_key' => array(
					...
					'name'    => sprintf( __( '%s License Key', 'naked-social-widget' ), $this->item_name ),
					'desc'    => '',
					'type'    => 'license_key',
					'options' => array(
	...
	public function activate_license() {
		...
		if ( ! isset( $_POST['naked_social_widget_settings'] ) ) {
			return;
		}
		...
		if ( ! current_user_can( 'manage_naked_social_widget_settings' ) ) {
			return;
		}

		// License key has been deleted - remove the status.
		if ( empty( $_POST['naked_social_widget_settings'][ $this->item_shortname . '_license_key' ] ) ) {
		...
		$license = sanitize_text_field( $_POST['naked_social_widget_settings'][ $this->item_shortname . '_license_key' ] );
	...
	public function deactivate_license() {
		...
		if ( ! isset( $_POST['naked_social_widget_settings'][ $this->item_shortname . '_license_key' ] ) ) {
			return;
		}

		if ( ! wp_verify_nonce( $_REQUEST[ $this->item_shortname . '_license_key-nonce' ], $this->item_shortname . '_license_key-nonce' ) ) {
			wp_die( __( 'Nonce verification failed', 'naked-social-widget' ), __( 'Error', 'naked-social-widget' ), array( 'response' => 403 ) );
		}
	...
	public function weekly_license_check() {
		...
		if ( ! empty( $_POST['naked_social_widget_settings'] ) ) {
	...
	public function notices() {
		...
		if ( is_object( $license ) && 'valid' !== $license->license && empty( $showed_invalid_message ) ) {
			if ( empty( $_GET['tab'] ) || 'licenses' !== $_GET['tab'] ) {
				$messages[] = sprintf(
					__( 'You have invalid or expired license keys for %1$s. Please go to the <a href="%2$s" title="Go to Licenses page">Licenses page</a> to correct this issue.', 'naked-social-widget' ),
					$this->item_name,
					admin_url( 'options-general.php?page=naked-social-widget' )
				);
	...
	public function plugin_row_license_missing( $plugin_data, $version_info ) {
		...
		if ( ( ! is_object( $license ) || 'valid' !== $license->license ) && empty( $showed_missing_key_message[ $this->item_shortname ] ) ) {
			echo '&nbsp;<strong><a href="' . esc_url( admin_url( 'options-general.php?page=naked-social-widget' ) ) . '">' . __( 'Enter valid license key for automatic updates.', 'naked-social-widget' ) . '</a></strong>';
